...
function getSettingsUrl(): string {
  const canonicalPath = (window as Window & {CANONICAL_PATH?: string})
    .CANONICAL_PATH ?? '';
  return `${window.location.origin}${canonicalPath}/settings/`;
}
...
async function fetchApiKeyFromBackend(
  ...
  try {
    ...
    if (!res || res.status === 404) {
      return {
        error: `Missing API Token. Make sure to set it at ${getSettingsUrl()}.`,
      };
    }
