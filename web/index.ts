...
async function fetchApiKeyFromBackend(
  ...
  try {
    ...
    if (typeof res === 'string') {
      ...
      return token ? { token } : { error: 'Missing Gemini API key' };
    }
...
@customElement('gr-gemini-config')
class GrGeminiConfig extends LitElement {
  ...
  static override get styles() {
    return css`
      ...
      .title {
        width: 15.5em; /* Match Gerrit's standard width */
...
function install(plugin: PluginApi) {
  ...
  console.log('Gemini plugin installed', GrGeminiConfig);
  ...
  plugin.registerCustomComponent('profile', 'gr-gemini-config');
}
