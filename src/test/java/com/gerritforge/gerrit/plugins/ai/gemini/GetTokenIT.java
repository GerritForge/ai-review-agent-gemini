...
public class GetTokenIT extends LightweightPluginDaemonTest {
  ...
  public void shouldGetTokenForSelf() throws Exception {
    ...
    RestResponse response = userRestSession.get(getAddTokenUri("self"));
    response.assertOK();
  ...
  public void shouldReturnNotFoundWhenTokenNotSet() throws Exception {
    userRestSession.get(getAddTokenUri("self")).assertNotFound();
  }
  ...
  public void shouldReturnForbiddenWhenGettingAnotherUserToken() throws Exception {
    ...
    adminRestSession.put(getAddTokenUri("self"), input).assertCreated();

    userRestSession.get(getAddTokenUri(admin.id().toString())).assertForbidden();
  }
