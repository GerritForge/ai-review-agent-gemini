...
public class VersionedAiUserData extends VersionedMetaData {
  ...
  protected void onLoad() throws IOException, ConfigInvalidException {
    ...
    token = Optional.ofNullable(Strings.emptyToNull(cfg.getString(SECTION, SUBSECTION, TOKEN_KEY)));
  }
  ...
  protected boolean onSave(CommitBuilder commit) throws IOException {
    ...
    checkLoaded();
    checkTokenSet();
    cfg.setString(SECTION, SUBSECTION, TOKEN_KEY, token.get());
  ...
  private void checkTokenSet() {
    checkState(token.isPresent(), "Gemini token must be present and non-empty");
  }
