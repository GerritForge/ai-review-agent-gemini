...
public class AddToken implements RestModifyView<AccountResource, AddToken.Input> {
  ...
  private final VersionedGeminiToken.Factory geminiTokenFactory;
  ...
  protected AddToken(
      ...
      VersionedGeminiToken.Factory geminiTokenFactory,
      Codec codec,
  ...
  public Response<?> apply(AccountResource resource, AddToken.Input input)
    ...
    VersionedGeminiToken geminiToken = geminiTokenFactory.create(accountId).load();
    if (!geminiToken.setToken(encryptedToken)) {
