...
public class AddToken implements RestModifyView<AccountResource, AddToken.Input> {
  ...
  public Response<?> apply(AccountResource resource, AddToken.Input input)
    ...
    ExternalId extId =
        extIdFactory.create(
            SCHEME_EXTERNAL, getFormattedUserToken(callerId, token, codec), accountId);
            ...
            u -> {
              ...
              for (ExternalId e : externalIds.byAccount(accountId)) {
                if (SCHEME_EXTERNAL.equals(e.key().scheme())
                    && e.key().id() != null
                    && e.key().id().startsWith(getUserTokenPrefix(callerId))) {
  ...
  static String getFormattedUserToken(Account.Id accountId, String token, PBECodec codec) {
    String userTokenPrefix = getUserTokenPrefix(accountId);
    return String.join("", userTokenPrefix, codec.encode(token));
  }

  private static String getUserTokenPrefix(Account.Id accountId) {
    return String.format("%s_%s_", GEMINI_PREFIX, accountId.get());
  }
}
